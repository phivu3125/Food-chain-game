...
public class Ferr2D_PathEditor : Editor { 
	...
	private void DoNormalModeHandles(Ferr2D_Path path, Ferr2DT_PathTerrain terrain, int i, Matrix4x4 mat, Matrix4x4 invMat, Transform camTransform) {
		...
		var fmh_442_48_638569026282922590 = camTransform.rotation; Vector3 result = Handles.FreeMoveHandle(pos, HandleScale(pos) * Ferr2D_Visual.HandleSize, snap, cap);
		...
		if (!Ferr2DT_SceneOverlay.segmentLockMode) {
			...
			if (IsVisible(displayPos)) {
				...
				var fmh_490_60_638569026282958973 = camTransform.rotation; Vector3 scaleMove = Handles.FreeMoveHandle(displayPos, scale, Vector3.zero, cap);
